use lpkg::backends::normalize_id;
use lpkg::config::Config;
use lpkg::core::identity::IdentityGraph;
...
fn export_manifest_serializes() {
    use lpkg::models::{ExportManifest, ManifestPackage};
    ...
    let manifest = ExportManifest {
        ...
        packages: vec![ManifestPackage {
            ...
            backend_id: "org.mozilla.firefox".into(),
            pin: None,
            version: Some("131.0".into()),
        }],
    };
    let json = serde_json::to_string(&manifest).unwrap();
    assert!(json.contains("org.mozilla.firefox"));
}
